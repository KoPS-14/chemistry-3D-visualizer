...
import { visualizeChemistry, checkHealth } from './services/api';
import type { MoleculeData, ReactionData, ReactantProduct3DData, VisualizeResponse } from './types/reaction';
...
export const App: React.FC = () => {
  ...
  // Initial load: check backend health and visualize ethanol by default
  useEffect(() => {
    ...
    try {
      ...
      if (res.status === 'success' && res.data) {
        ...
        if (res.request_type === 'molecule') {
          ...
          // Select first reactant by default to show in 3D viewer
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-white">
      ...
      <header className="border-b border-slate-800 bg-slate-900/60 backdrop-blur-md px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">AI-Powered Chemistry Visualization System</p>
          </div>
        </div>
      ...
      <footer className="border-t border-slate-800/60 py-4 px-6 text-center text-xs text-slate-500">
        AI-Powered Chemistry Visualization System • Powered by FastAPI & RDKit 3D Force Field Optimization
      </footer>
    </div>
  );
};
